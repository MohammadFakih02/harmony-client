 ...
 * account credentials summary, and Log Out. Replaces the old separate Profile pane (the two
 * showed the same card twice). Credential changes (password/email/username) are a future slice.
 */
@Component({
  selector: 'app-account-settings',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [FormsModule, UiAvatar, UiButton, UiProfileBanner],
  template: `
    ...
    <!-- Account credentials (read-only until the credential-change slice lands) -->
...
export class AccountSettings implements OnInit {
  ...
  /** Email lives on the auth session, not the editable-profile payload. */
